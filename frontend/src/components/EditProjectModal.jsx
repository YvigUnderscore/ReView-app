...
const EditProjectModal = ({ project, onClose, onProjectUpdated, onProjectDeleted }) => {
    ...
    const [selectedRoleIds, setSelectedRoleIds] = useState(project.roles ? project.roles.map(r => r.id) : []);

    React.useEffect(() => {
        // Fetch Team Roles
        if (project.teamId) {
            fetch(`/api/teams/${project.teamId}/roles`, {
    ...
    const toggleRole = async (roleId) => {
        ...
        setSelectedRoleIds(newRoleIds); // Optimistic update
        ...
        try {
            ...
            if (res.ok) {
                // Update parent so the card updates immediately
                const updatedRoles = teamRoles.filter(r => newRoleIds.includes(r.id));
                onProjectUpdated({ ...project, roles: updatedRoles });
            } else {
                // Revert on error
                            ...
                            {activeTab === 'roles' && (
                                <div className="space-y-4">
                                    <div className="border border-border rounded-lg p-4 bg-muted/20">
                                        <h3 className="text-sm font-medium mb-3">Project Roles</h3>
                                        {teamRoles.length > 0 ? (
                                            ...
                                            <p className="text-xs text-muted-foreground">No roles defined in this team.</p>
                                        )}
                                    </div>
                                    <p className="text-xs text-muted-foreground px-1">
                                        Active roles determine which department channels this project notifies.
                                    </p>
                                </div>
                            )}
